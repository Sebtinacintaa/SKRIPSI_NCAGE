...
function OtpBoxes({ otp, setOtp, disabled }: { otp: string; setOtp: (v: string) => void; disabled?: boolean }) {
...
export default function ForgotPasswordView() {
  ...
  const doSendOtp = (email: string) => {
    ...
    startSendTransition(async () => {
      ...
      else if (result.success && result.email) { setSentEmail(result.email); setStep("otp"); setOtp(""); setSecs(60); }
    });
  };
  ...
  return (
    <div className="min-h-screen flex flex-col items-center justify-start pt-32 pb-12 bg-[#F5EEE8] relative overflow-hidden">
      ...
      <div className="bg-white w-full max-w-[560px] p-10 sm:p-14 rounded-[15px] shadow-xl z-10 border border-gray-100 animate-in fade-in zoom-in-95 slide-in-from-bottom-4 duration-700 ease-out">
        <div className="mb-8">
          {step === "otp" ? (
            <button onClick={() => { setStep("email"); setOtp(""); setSecs(0); }} className="inline-flex items-center text-[#8B1E1E] font-medium hover:opacity-70 transition-opacity gap-2">
              <i className="ri-arrow-left-line text-xl" /><span className="text-sm">Kembali</span>
            </button>
          ) : (
            <Link href="/login" className="inline-flex items-center text-[#8B1E1E] font-medium hover:opacity-70 transition-opacity gap-2">
              <i className="ri-arrow-left-line text-xl" /><span className="text-sm">Kembali ke Login</span>
            </Link>
          )}
        </div>
        {step === "email" && (
            ...
            <form onSubmit={handleEmailSubmit} className="flex flex-col gap-6">
              <div>
                ...
                <div className="relative group">
                  ...
                  <input type="email" name="email" required placeholder="Masukkan alamat email Anda" className="w-full pl-12 pr-4 py-3.5 border border-[#E5E7EB] rounded-xl text-[#374151] font-medium placeholder:font-normal placeholder:text-gray-400 focus:outline-none focus:ring-2 focus:ring-[#8B1E1E]/20 focus:border-[#8B1E1E] transition-all" />
                </div>
                {sendError && <p className="mt-2 text-xs text-red-500 font-medium flex items-center gap-1 animate-in slide-in-from-top-1"><i className="ri-error-warning-line" /> {sendError}</p>}
              </div>
              <button type="submit" disabled={sendPending} className="w-full bg-[#5D3A3A] text-white py-4 rounded-xl font-bold text-base hover:bg-[#4a2e2e] transition-all active:scale-[0.98] shadow-lg shadow-[#5D3A3A]/10 disabled:opacity-60 disabled:cursor-not-allowed flex items-center justify-center gap-2">
                {sendPending ? <><i className="ri-loader-4-line animate-spin" /> Mengirim...</> : <><i className="ri-send-plane-line" /> Kirim Kode OTP</>}
              </button>
            </form>
          </>
        )}
        {step === "otp" && (
          <>
            <div className="text-center mb-8">
              ...
              <p className="text-gray-500 text-sm">Kode 6 digit telah dikirim ke <span className="font-semibold text-[#374151]">{sentEmail}</span></p>
            </div>
            <form action={otpAction} className="flex flex-col gap-6">
              <input type="hidden" name="email" value={sentEmail} />
              <input type="hidden" name="otp" value={otp} />
              <OtpBoxes otp={otp} setOtp={setOtp} disabled={otpPending} />
              {otpState?.error && <p className="text-center text-xs text-red-500 font-medium flex items-center justify-center gap-1 animate-in slide-in-from-top-1"><i className="ri-error-warning-line" /> {otpState.error}</p>}
              {sendError && <p className="text-center text-xs text-red-500 font-medium flex items-center justify-center gap-1"><i className="ri-error-warning-line" /> {sendError}</p>}
              <button type="submit" disabled={otpPending || otp.replace(/\s/g, "").length !== 8} className="w-full bg-[#5D3A3A] text-white py-4 rounded-xl font-bold text-base hover:bg-[#4a2e2e] transition-all active:scale-[0.98] shadow-lg shadow-[#5D3A3A]/10 disabled:opacity-40 disabled:cursor-not-allowed flex items-center justify-center gap-2">
            ...
            <div className="text-center mt-6 text-sm text-gray-500">
              Tidak menerima kode?{" "}
              {secs > 0 ? <span className="text-gray-400 font-medium">Kirim ulang dalam {secs}d</span>
                : <button onClick={() => doSendOtp(sentEmail)} disabled={sendPending} className="text-[#8B1E1E] font-bold hover:opacity-80 transition-opacity disabled:opacity-50">Kirim Ulang</button>}
            </div>
          </>
        )}
      </div>
